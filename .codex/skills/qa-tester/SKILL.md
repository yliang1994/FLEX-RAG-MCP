...
> Pick ONE test → Run ONE command → Wait for output → Record ONE row in `.codex/skills/qa-tester/QA_TEST_PROGRESS.md` → THEN pick next.
...
6. ⛔ GATE: Edit `.codex/skills/qa-tester/QA_TEST_PROGRESS.md` (row + counters) — ONE row per edit
...
1. Read `.codex/skills/qa-tester/QA_TEST_PLAN.md` for test steps and expected results.
2. Read `.codex/skills/qa-tester/QA_TEST_PROGRESS.md` for current status.
...
1. Read the test's **Steps** column from `.codex/skills/qa-tester/QA_TEST_PLAN.md`.
...
For tests NOT in the script, run commands from `.codex/skills/qa-tester/QA_TEST_PLAN.md` manually and paste output.
...
Edit `.codex/skills/qa-tester/QA_TEST_PROGRESS.md`: update ONE test row + summary counters. ONE row per file edit.
...
| File | Purpose |
|------|---------|
| `.codex/skills/qa-tester/QA_TEST_PLAN.md` | Test steps and expected results |
| `.codex/skills/qa-tester/QA_TEST_PROGRESS.md` | Execution status and notes |
