...
| `Baseline` | 标准数据：default 集合(simple.pdf + with_images.pdf)、test_col 集合(complex_technical_doc.pdf)、有 Trace | `python .codex/skills/qa-tester/scripts/qa_bootstrap.py` |
...
| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| K-01 | settings.yaml 切换 LLM 到 DeepSeek | DeepSeek | 1. 执行 `python .codex/skills/qa-tester/scripts/qa_config.py apply deepseek`<br>2. 检查输出确认切换成功 | 输出显示 "LLM -> deepseek / deepseek-chat"，settings.yaml 已更新 |
...
| K-09 | DeepSeek 回退 Azure 验证 | DeepSeek | 1. 执行 `python .codex/skills/qa-tester/scripts/qa_config.py restore`<br>2. 执行 `python scripts/query.py --query "Sample Document PDF loader" --verbose` | 功能恢复正常，Verbose 输出显示使用 Azure LLM，查询结果包含 simple.pdf 相关内容，验证切换回来无副作用 |
...
# 步骤 3: 验证配置
python .codex/skills/qa-tester/scripts/qa_config.py check
```
...
```powershell
# 查看可用 Profile
python .codex/skills/qa-tester/scripts/qa_config.py show

# 切换到 DeepSeek（自动备份 settings.yaml，注入 API Key）
python .codex/skills/qa-tester/scripts/qa_config.py apply deepseek

# 执行测试...

# 测试完成后恢复原始配置
python .codex/skills/qa-tester/scripts/qa_config.py restore
```
